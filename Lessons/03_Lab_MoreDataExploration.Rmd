...
> Answer: The different column headers appear. 
...
```{r}
dataframe1$ID
hightemp <- c(48,47,44,39,37,37,36,49,45,46)
lowtemp <- c(40,34,29,28,19,24,29,30,27,27)
```
...
```{r}
fartocelcius <- function(x){
  f2c <- (x-32)*(5/9)
  return(f2c)
}
celciushigh <- fartocelcius(hightemp)
celciuslow <- fartocelcius(lowtemp)
```
...
tempdataframe <- cbind(hightemp, lowtemp, celciushigh, celciuslow)
colnames(tempdataframe) <- c("highfar", "lowfar", "highcel", "lowcel")
