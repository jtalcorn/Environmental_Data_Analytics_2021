...
HUC8_sf_filter<-st_read("./Data/Spatial/NCHUC8.shp")%>%
  filter(SUBBASIN!= "Upper Neuse")


#Create a map
mapview(HUC8_sf, col.regions = "orange")+
  mapview(HUC8_sf_filter, col.regions = "purple")
...
wind_energy_huc8<-read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c("HUC_12"="factor"))%>%
  ...
  group_by(HUC_8)%>%
  summarise(power=mean(kWhkm2day))


#Join to HUC_8 features
wind_HUC8_Join<-left_join(HUC8_sf, wind_energy_huc8, by = c("HUC_8"))



#View the outputs

mapview(wind_HUC8_Join, zcol = "power")
