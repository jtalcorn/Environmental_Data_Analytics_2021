...
pmozone<-lm(data = EPAair, Ozone ~ PM2.5)
summary(pmozone)
...
newEPAair<- 
  EPAair%>%
  select(Month, PM2.5, Ozone, Year, SITE_LATITUDE, SITE_LONGITUDE)%>%
  na.omit()
corEPAair<- cor(newEPAair)
corrplot(corEPAair, method = "ellipse")
...
lmEPAair<-lm(data = newEPAair, Ozone~PM2.5+Month+SITE_LATITUDE)
step(lmEPAair)



#Exercise 7: Run another regression using the variables selected on Exercise 6. Compare r-squared value withthe one from Exercise 5. 
